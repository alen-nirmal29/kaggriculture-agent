"""Read-only state abstraction for Kaggriculture observations."""
...
@dataclass(frozen=True)
class GameState:
    """Normalized, immutable references to one player's public/private state."""
    ...
    @property
    def empty_tiles(self) -> tuple[TileView, ...]:
        return tuple(tile for tile in self.iter_tiles() if tile.is_empty)

    @property
    def weed_tiles(self) -> tuple[TileView, ...]:
        return tuple(tile for tile in self.iter_tiles() if tile.kind == "WEED")

    @property
    def crop_tiles(self) -> tuple[TileView, ...]:
        return tuple(tile for tile in self.iter_tiles() if tile.kind == "PLANT")
