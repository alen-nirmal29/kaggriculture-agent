"""Independent deterministic routing helpers copied from proven V1 behavior."""

from agent_v2.state import Position
...
def next_move(start: Position, target: Position, board_size: int = 10) -> str:
    sx, sy = start
    tx, ty = target
    if not (0 <= sx < board_size and 0 <= sy < board_size and 0 <= tx < board_size and 0 <= ty < board_size):
        return "PASS"
    if tx < sx:
        return "WEST"
    if tx > sx:
...
def nearest_position(origin: Position, positions: list[Position]) -> Position | None:
    return min(positions, key=lambda p: (manhattan_distance(origin, p), p[1], p[0])) if positions else None
